...
function Library:MakeDraggableUnrestricted(Instance)
    Instance.Active = true;
    Instance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 then
            local ObjPos = Vector2.new(
...
function Library:CreateWindow(...)
    ...
    Library:AddToRegistry(MinimizeButton, { BackgroundColor3 = 'MainColor'; BorderColor3 = 'OutlineColor'; TextColor3 = 'FontColor'; });

    local MinimizedCircle = Library:Create('TextButton', {
        AnchorPoint = Vector2.new(0.5, 0.5);
        BackgroundColor3 = Library.AccentColor;
        BorderColor3 = Library.AccentColorDark;
        BorderMode = Enum.BorderMode.Inset;
        Position = Config.Position + UDim2.fromOffset(25, 25);
    ...
    Library:AddToRegistry(MinimizedCircle, { BackgroundColor3 = 'AccentColor'; BorderColor3 = 'AccentColorDark'; });

    Library:MakeDraggableUnrestricted(MinimizedCircle);
    ...
    local function SetMinimized(State)
        ...
        if State then
            ...
            MinimizedCircle.Position = UDim2.fromOffset(cx, cy);
            TweenService:Create(Outer, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            ...
            local cx = MinimizedCircle.AbsolutePosition.X;
            local cy = MinimizedCircle.AbsolutePosition.Y;
            Outer.Position = UDim2.fromOffset(cx - Config.Size.X.Offset / 2, cy - Config.Size.Y.Offset / 2);
            Outer.Size = UDim2.fromOffset(0, 0);
            Outer.Visible = true;
            TweenService:Create(Outer, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = Config.Size;
            }):Play();
    ...
    Library:OnHighlight(MinimizeButton, MinimizeButton,
