...
-- ══════════════════════════════════════════════════════════
--  MINIMIZED FLOATING PANEL  (library-native style)
-- ══════════════════════════════════════════════════════════
-- Matches the library exactly:
--   outer Frame  → black border  (BorderColor3 = black)
--   inner Frame  → MainColor bg, OutlineColor inset border
--   UIGradient   → white→212,212,212 at 90°  (same as buttons)
--   accent bar   → 2px AccentColor top strip
--   icon         → drawn with Frames, no images needed
--   hover        → OutlineColor border → AccentColor border
-- Size: 120×28  (compact, fits in a corner)
-- ══════════════════════════════════════════════════════════
local _miniSg    = nil
local _miniPanel = nil   -- the outer Frame (for drag)
local _miniShown = false

local function _drawMinimizeIcon(parent, zBase)
    -- Three horizontal lines (hamburger / restore icon)
    -- drawn as plain Frames, theme-colored via registry
    local lineH = 2
    local lineW = 10
    local gap   = 3
    local totalH = lineH * 3 + gap * 2
    local startY = math.floor((12 - totalH) / 2)  -- vertically centred in 12px icon area

    for i = 0, 2 do
        local line = Instance.new('Frame')
        line.BackgroundColor3 = Library.FontColor
        line.BorderSizePixel  = 0
        line.Size             = UDim2.fromOffset(lineW, lineH)
        line.Position         = UDim2.fromOffset(
            math.floor((12 - lineW) / 2),
            startY + i * (lineH + gap)
        )
        line.ZIndex  = zBase + 2
        line.Parent  = parent
        Library:AddToRegistry(line, { BackgroundColor3 = 'FontColor' })
    end
end
...
local function _buildMiniSystem()
    ...
    sg.Name           = 'LibMiniPanel'
    sg.ResetOnSpawn   = false
    sg.ZIndexBehavior = Enum.ZIndexBehavior.Global
    ProtectGui(sg)
    sg.Parent = CoreGui

    -- ── outer shell (black border, same as all library frames) ──
    local panelOuter = Instance.new('Frame')
    panelOuter.Name             = 'MiniOuter'
    panelOuter.BackgroundColor3 = Color3.new(0, 0, 0)
    panelOuter.BorderSizePixel  = 0
    panelOuter.Size             = UDim2.fromOffset(122, 30)
    panelOuter.Position         = UDim2.fromOffset(16, 200)
    panelOuter.ZIndex           = 120
    panelOuter.Visible          = false
    panelOuter.Active           = true
    panelOuter.Parent           = sg

    Instance.new('UICorner', panelOuter).CornerRadius = UDim.new(0, 4)

    -- ── inner frame (MainColor bg + OutlineColor inset border) ──
    local panelInner = Instance.new('Frame')
    panelInner.BackgroundColor3 = Library.MainColor
    panelInner.BorderColor3     = Library.OutlineColor
    panelInner.BorderMode       = Enum.BorderMode.Inset
    panelInner.Size             = UDim2.new(1, -2, 1, -2)
    panelInner.Position         = UDim2.fromOffset(1, 1)
    panelInner.ZIndex           = 121
    panelInner.Parent           = panelOuter

    Instance.new('UICorner', panelInner).CornerRadius = UDim.new(0, 3)

    Library:AddToRegistry(panelInner, { BackgroundColor3 = 'MainColor'; BorderColor3 = 'OutlineColor' })

    -- ── subtle white→grey gradient (identical to library buttons) ──
    ...
    divider.Position         = UDim2.fromOffset(28, 5)
    divider.ZIndex           = 122
    divider.Parent           = panelInner
    Library:AddToRegistry(divider, { BackgroundColor3 = 'OutlineColor' })

    -- ── label "Minimized" ──
    local lbl = Instance.new('TextLabel')
    lbl.BackgroundTransparency = 1
    lbl.Size                   = UDim2.fromOffset(80, 28)
    lbl.Position               = UDim2.fromOffset(33, 0)
    ...
    lbl.ZIndex                 = 122
    lbl.Parent                 = panelInner
    Library:AddToRegistry(lbl, { TextColor3 = 'FontColor' })

    -- ── invisible click region covers entire panel ──
    local clickRegion = Instance.new('TextButton')
    clickRegion.BackgroundTransparency = 1
    clickRegion.Size                   = UDim2.fromScale(1, 1)
    clickRegion.Text                   = ''
    clickRegion.ZIndex                 = 125
    clickRegion.Parent                 = panelOuter

    -- ── hover: outer border colour changes to AccentColor ──
    clickRegion.MouseEnter:Connect(function()
        panelOuter.BackgroundColor3 = Library.AccentColor
    end)
    ...
    clickRegion.InputBegan:Connect(function(inp)
        if inp.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
        dragging   = true
        didMove    = false
        dragOffset = Vector2.new(
            inp.Position.X - panelOuter.AbsolutePosition.X,
            inp.Position.Y - panelOuter.AbsolutePosition.Y
        )
    end)
    ...
    InputService.InputChanged:Connect(function(inp)
        ...
        panelOuter.Position = UDim2.fromOffset(
            math.clamp(inp.Position.X - dragOffset.X, 0, vp.X - 122),
            math.clamp(inp.Position.Y - dragOffset.Y, 0, vp.Y - 30)
        )
    end)

    clickRegion.InputEnded:Connect(function(inp)
        if inp.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
        dragging = false
        if not didMove then
            panelOuter.Visible = false
...
function Library:CreateWindow(...)
    ...
    -- ── MINIMIZE BUTTON (title bar, library-native style) ───
    -- Outer: black border frame   (same as all library elements)
    -- Inner: MainColor + OutlineColor inset + white/grey gradient
    -- Hover: border → AccentColor  (same as toggle/slider hover)
    -- Icon:  two pixel-art lines = classic minimise symbol
    do
        -- outer shell — black border like every other library frame
        local MinOuter = Instance.new('Frame')
        MinOuter.Name            = 'MinimizeBtnOuter'
        MinOuter.BackgroundColor3 = Color3.new(0, 0, 0)
        MinOuter.BorderSizePixel  = 0
        MinOuter.Size             = UDim2.fromOffset(19, 19)
        MinOuter.Position         = UDim2.new(1, -24, 0, 3)
        MinOuter.ZIndex           = 8
        MinOuter.Parent           = Inner
        Instance.new('UICorner', MinOuter).CornerRadius = UDim.new(0, 3)
        Library:AddToRegistry(MinOuter, { BackgroundColor3 = 'Black' })

        -- inner fill — MainColor + OutlineColor inset border
        local MinInner = Instance.new('Frame')
        MinInner.BackgroundColor3 = Library.MainColor
        MinInner.BorderColor3     = Library.OutlineColor
        MinInner.BorderMode       = Enum.BorderMode.Inset
        MinInner.Size             = UDim2.new(1, -2, 1, -2)
        MinInner.Position         = UDim2.fromOffset(1, 1)
        MinInner.ZIndex           = 9
        MinInner.Parent           = MinOuter
        Instance.new('UICorner', MinInner).CornerRadius = UDim.new(0, 2)
        Library:AddToRegistry(MinInner, { BackgroundColor3 = 'MainColor'; BorderColor3 = 'OutlineColor' })

        -- white→grey gradient (identical to library buttons/sliders)
        local minGrad = Instance.new('UIGradient')
        minGrad.Color    = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.new(1, 1, 1)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(212, 212, 212)),
        })
        minGrad.Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, 0.86),
            NumberSequenceKeypoint.new(1, 0.90),
        })
        minGrad.Rotation = 90
        minGrad.Parent   = MinInner

        -- minimise icon: single thick horizontal bar + thin bottom line
        --   bar  (3px tall, centred)  — the universal "minimise" symbol
        --   line (1px tall, bottom-2) — depth hint
        local bar = Instance.new('Frame')
        bar.BackgroundColor3 = Library.FontColor
        bar.BorderSizePixel  = 0
        bar.Size             = UDim2.fromOffset(11, 2)
        bar.Position         = UDim2.fromOffset(3, 7)
        bar.ZIndex           = 10
        bar.Parent           = MinInner
        Library:AddToRegistry(bar, { BackgroundColor3 = 'FontColor' })

        local barLine = Instance.new('Frame')
        barLine.BackgroundColor3 = Library.OutlineColor
        barLine.BorderSizePixel  = 0
        barLine.Size             = UDim2.fromOffset(11, 1)
        barLine.Position         = UDim2.fromOffset(3, 12)
        barLine.ZIndex           = 10
        barLine.Parent           = MinInner
        Library:AddToRegistry(barLine, { BackgroundColor3 = 'OutlineColor' })

        -- click region (transparent, sits on top)
        local MinClick = Instance.new('TextButton')
        MinClick.BackgroundTransparency = 1
        MinClick.Size                   = UDim2.fromScale(1, 1)
        MinClick.Text                   = ''
        MinClick.ZIndex                 = 11
        MinClick.Parent                 = MinOuter

        -- hover: outer border flashes AccentColor (same as slider/toggle hover)
        MinClick.MouseEnter:Connect(function()
            MinOuter.BackgroundColor3 = Library.AccentColor
        end)
        MinClick.MouseLeave:Connect(function()
            MinOuter.BackgroundColor3 = Color3.new(0, 0, 0)
        end)
